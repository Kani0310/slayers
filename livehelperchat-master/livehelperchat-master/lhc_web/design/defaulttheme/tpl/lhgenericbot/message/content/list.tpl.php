<div class="list-group meta-message meta-message-<?php echo $messageId?>">
                            ...
                            <ul class="quick-replies list-inline">
                                <?php foreach ($item['buttons'] as $itemButton) : ?>
                                    <li class="list-inline-item">
                                        ...
                                        <a rel="noreferrer" class="btn btn-xs btn-outline-primary btn-bot" <?php if (isset($itemButton['content']['payload_message']) && $itemButton['content']['payload_message'] != '') : ?>data-payload=<?php echo json_encode($itemButton['content']['payload_message'])?> data-id="<?php echo $messageId?>" onclick='lhinst.buttonClicked(<?php echo json_encode($itemButton['content']['payload_message'])?>,<?php echo $messageId?>,$(this))'<?php endif;?> target="_blank" href="<?php echo htmlspecialchars($itemButton['content']['payload'])?>">
                                        ...
                                        <a rel="noreferrer" class="btn btn-sm btn-outline-primary btn-bot" data-no-change="true" data-id="<?php echo $messageId?>" data-payload=<?php echo json_encode($itemButton['content']['payload'])?> data-keep="true" onclick='lhinst.updateChatClicked(<?php echo json_encode($itemButton['content']['payload'])?>,<?php echo $messageId?>,$(this),true)'>
                                        <?php elseif ($itemButton['type'] == 'trigger') : ?>
                                        <a rel="noreferrer" class="btn btn-sm btn-outline-primary btn-bot" data-no-change="true" data-id="<?php echo $messageId?>" data-payload=<?php echo json_encode($itemButton['content']['payload']. '__' . md5($itemButton['content']['name']))?> data-keep="true" onclick='lhinst.updateTriggerClicked(<?php echo json_encode($itemButton['content']['payload']. '__' . md5($itemButton['content']['name']))?>,<?php echo $messageId?>,$(this),true)'>
                                        <?php else : ?>
                                        <a rel="noreferrer" class="btn btn-sm btn-outline-primary btn-bot" data-no-change="true" data-id="<?php echo $messageId?>" data-payload=<?php echo json_encode($itemButton['content']['payload']. '__' . md5($itemButton['content']['name']))?> data-keep="true" onclick='lhinst.buttonClicked(<?php echo json_encode($itemButton['content']['payload']. '__' . md5($itemButton['content']['name']))?>,<?php echo $messageId?>,$(this),true)'>
                                        <?php endif?>
    ...
    <div class="list-group-element button-item <?php $listCompactStyle == true ? print ' compact' : print ' large'?>">
        ...
        <a rel="noreferrer" target="_blank" <?php if (isset($item['content']['payload_message']) && $item['content']['payload_message'] != '') : ?>onclick='lhinst.buttonClicked(<?php echo json_encode($item['content']['payload_message'])?>,<?php echo $messageId?>,$(this))'<?php endif;?> href="<?php echo htmlspecialchars($item['content']['payload'])?>">
        <i class="material-icons"><?php if (isset($react) && $react == true) : ?>&#xf106;<?php else : ?>open_in_new<?php endif; ?></i>
